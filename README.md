...
| libraryCardComponent | React.FC                                   | [FileLibraryCard](src/components/FileLibraryCard/FileLibraryCard.tsx) | Custom rendering component for the card in the library tab. See [FileLibraryCard](src/components/FileLibraryCard/FileLibraryCard.tsx) as an example.          |
...
import {FileLibraryListItem, ReactMediaLibrary, convertFileToBase64} from 'react-media-library';
...
const ReactMediaLibraryWrapper: React.FC = () => {
    ...
    async function uploadCallback(file: File): Promise<boolean> {
        ...
        const result = await fileUpload(fileBase64);
